...
def create_malbork_graph():
    ...
    graph.create_node(BoundingBox([6, 7, 8, 9]), GeoCoords(2, 6), u"Braniewo")
    gdansk = graph.get_nodes_by_name(u"Gdansk")
    braniewo = graph.get_nodes_by_name(u"Braniewo")
    gdansk.push_sym_edge(braniewo, 100)
    malbork = Node(BoundingBox([4, 3, 2, 1]), GeoCoords(1, 1), u"Malbork")
...
def create_real_graph(graph_file=u"real_shit2", lookup_file=None, base_graph=None, base_lookup=None):
    ...
    for i, row in enumerate(freight_steps):
        if i%3 == 0 and i!=0:
            ...
            print u'Autosave on iteration: ', i
    ...
    if lookup_file is not None:
        with open(lookup_file, u"wb") as f:
            pickle.dump(steps_lookup, f)
        print u'Autosaved location lookup table'
    return graph
